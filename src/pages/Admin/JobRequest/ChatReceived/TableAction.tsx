...
import { ISentApplicationAdmin } from 'service/service-offer-org';
...
type SentData = ISentApplicationAdmin['data'][number];
      ...
      <Button
        ...
        onClick={() =>
          props.onStatusChange({
            id: props.row.original.vacancy_application[0].id,
          })
        }
      >
        Change Status
...
export const useReceivedChatColumnn = () => {
  const columns = useMemo<ColumnDef<SentData, any>[]>(
    () => [
        ...
        cell: ({ row }) => (
          <Link
            to={NAVIGATION_ROUTES.VACANCY.DETAILS.replace(
              ':id',
              row.original.vacancy_application[0].vacancy.id,
            )}
          >
            <Text cursor="pointer" color="brand.blueActive">
              {row.original.vacancy_application[0].vacancy.name}
            </Text>
          </Link>
        ),
      },
      {
        ...
        cell: ({ row }) => {
          const name =
            row.original.teacher.user_profile.first_name +
            ' ' +
            row.original.teacher.user_profile.last_name;
          return (
            <Tooltip label={name}>
              <Link
                to={NAVIGATION_ROUTES.TEACHERDETAILS.replace(
                  ':id',
                  row.original.teacher.teacher.id,
                )}
              >
                <Avatar
                  size={'sm'}
                  name={name}
                  src={
                    imageURL + row.original.teacher.user_profile.profile_picture
                  }
                />
              </Link>
            </Tooltip>
        ...
        cell: ({ row }) => (
          <Text>
            <Badge colorScheme="green">
              {row.original.teacher.teacher.no_of_periods} Periods
            </Badge>
          </Text>
        ),
      },
        ...
        cell: ({ row }) => (
          <Text>
            <Badge colorScheme="purple">
              rs. {row.original.teacher.teacher.salary_per_period}
            </Badge>
          </Text>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <Badge color="green">
            {row.original.teacher.teacher.experience_in_years} years
          </Badge>
        ),
      },
      {
        ...
        cell: ({ row }) => {
          const fileUrl = imageURL + 'media/' + row.original.teacher.cv[0].file;
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Box>
            {row.original.teacher.teacher.is_available_for_tution ? (
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Box>
            {row.original.teacher.teacher.can_shift_location ? (
              <FaCheck color="green" fontSize={'30px'} />
            ) : (
              <RxCross1 color="red" fontSize={'30px'} />
            )}
